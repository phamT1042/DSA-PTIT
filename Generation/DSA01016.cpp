...
void phanTich(int a[]) {
    int i = cnt;
    while (i > 0 && a[i] == 1) i--; //Cấu hình cuối cùng gồm toàn số 1
                                    //Tìm vị trí i có a[i] > 1 để giảm và phân tích tiếp
    if (!i) { //Cấu hình cuối
        flag = 1;
        return;
    }
    ...
    int q = number_of_digits_1 / a[i]; //Phân tích số chữ số 1 thành a[i] bằng chia nguyên cho a[i]
    int r = number_of_digits_1 % a[i]; //Phân tích số chữ số 1 thành 1 bằng chia dư cho a[i]
    while (q--) {
        cnt++;
        a[cnt] = a[i];
    }
